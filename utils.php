...
function authors_from_string($authorstring, $split_on_commas = false)
{
	...
	$authorstring = preg_replace("/ et /iu", "|", $authorstring);
    ...
    */
	...
	{
		$authors[$i] = preg_replace('/\.(\p{Lu})/u', ". $1", $authors[$i]);
...
//----------------------------------------------------------------------------------------
function reference_from_matches($matches, &$reference = null, $split_on_commas = false)
{
	if ($reference == null)
	{
	...
	if (isset($matches['authorstring']))
	{
		...
		$reference->authors = authors_from_string($authorstring, $split_on_commas);
